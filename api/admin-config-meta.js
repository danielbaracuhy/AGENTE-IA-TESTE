...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    const { cliente_id, meta_ad_account_id, meta_page_id, meta_business_id, whatsapp } = req.body || {};
    if (!cliente_id) return res.status(400).json({ erro: 'cliente_id é obrigatório.' });

    const payload = { meta_ad_account_id, meta_page_id, meta_business_id, whatsapp };

    try {
      // Verifica se já existe uma linha para esse cliente
