...
export async function getMetaConfig(req) {
  ...
  if (token && SUPABASE_URL && SUPABASE_SECRET) {
    try {
      ...
      const cfgRes = await fetch(
        `${SUPABASE_URL}/rest/v1/meta_config?cliente_id=eq.${cli.id}&ativo=eq.true&select=meta_ad_account_id,meta_page_id&limit=1`,
        { headers: { Authorization: `Bearer ${SUPABASE_SECRET}`, apikey: SUPABASE_SECRET, Accept: 'application/json' } }
      );
  ...
  return { adAccountId: process.env.META_AD_ACCOUNT_ID, pageId: process.env.META_PAGE_ID };
}
